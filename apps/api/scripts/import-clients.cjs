/*
 * Script CLI d'import Minos (clients) — sans HTTP ni auth.
 * Réutilise le même parser / mapping / validation que le worker BullMQ.
 *
 * Usage :  node scripts/import-clients.cjs <chemin_fichier>
 *   (ou)   pnpm --filter @crm/api import:clients <chemin_fichier>
 */
...
const { clientFieldSpec } = require('../dist/import/mappings/client.mapping');
const { clientRowSchema, toMinosClientData } = require('../dist/import/mappings/client.schema');
...
async function main() {
  ...
  if (!file) {
    console.error('Usage: node scripts/import-clients.cjs <chemin_fichier>');
    process.exit(1);
  }
  ...
  const flush = async () => {
    ...
    await prisma.$transaction(
      rows.map((d) =>
        prisma.client.upsert({ where: { codeAs400: d.codeAs400 }, create: d, update: d }),
      ),
    );
  };
  ...
  console.log(`\nImport ${status} en ${secs}s — total=${total} ok=${ok} failed=${failed} (log ${log.id})`);
